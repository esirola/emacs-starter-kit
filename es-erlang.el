...
(setq erlang-root-dir
      (shell-command-to-string
       "erl -noinput -eval \"io:format(\\\"~s\\\", [code:root_dir()])\" -run init stop")
      erlang-load-path
      (concat (shell-command-to-string
               "erl -noinput -eval \"io:format(\\\"~s\\\", [code:lib_dir(tools)])\" -run init stop")
              "/emacs")
      erlang-root-man-dir (expand-file-name (concat erlang-root-dir "/../../share/man/")))
...
(ignore-errors
  ...
  (defun my-erlang-mode-hook ()
    ...
    (setq inferior-erlang-machine-options '("-mnesia" "dir" "\"/tmp/mnesia-store\"" "-sname" "emacs"))
